let init = () => {
    ...
    let startSliding = () => {
        ...
        setInterval(() => {
            ...
            // remove from active from first child and add it to the second slide so it can become the first side with the class activiated
            slides[1].classList.add('jumbo-slider__slide--active')
            slides[0].classList.remove('jumbo-slider__slide--active')
            // clone the first slide and placed on the last space.
            container.appendChild(slides[0].cloneNode([true]));
            container.removeChild(slides[0]);
            // then remove the first slide after it has ben cloned
